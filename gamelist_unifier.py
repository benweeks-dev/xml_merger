    ...
    def standardize_media_paths(self, preferred_structure: str = 'retrobat') -> None:
        ...
        for rom_path, entry in self.games.items():
            ...
            if preferred_structure == 'retrobat':
                ...
                # ALWAYS standardize ALL media paths to match ROM subfolder structure
...
💾 Generating unified gamelist: {output_path}")
    ...
    def _prettify_and_save(self, elem: ET.Element, filepath: Path) -> None:
        """Save XML with proper indentation and explicit closing tags"""
        ...
        # Convert self-closing tags to explicit closing tags
        # <tag/> becomes <tag></tag>
        final_lines = []
        for line in lines:
            # Match self-closing tags like <rating/> or <desc />
            import re
            line = re.sub(r'<(\w+)\s*/>', r'<\1></\1>', line)
