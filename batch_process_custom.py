...
def process_all_systems():
    ...
    with open(index_path, 'w', encoding='utf-8') as f:
        ...
        for system_info in systems_to_process:
            system = system_info['system']
            info = results[system]
            ...
            # Show paths
            f.write(f"\n  Paths:\n")
            f.write(f"    RetroBat:  {system_info['retrobat_base']}\n")
            if system_info['pc_backup_base']:
                f.write(f"    PC Backup: {system_info['pc_backup_base']}\n")
            else:
                f.write(f"    PC Backup: Not configured\n")
            ...
            if info['status'] == 'success':
                ...
                f.write(f"\n  Deploy To:\n")
                f.write(f"    RetroBat:  {system_info['retrobat_base']}\\{system}_unified_gamelist.xml\n")
                if system_info['pc_backup_base']:
                    f.write(f"    PC Backup: {system_info['pc_backup_base']}\\media\\{system}_unified_gamelist.xml\n")
